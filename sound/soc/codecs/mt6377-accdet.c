...
static struct mt6377_accdet_data *accdet;
...
static struct accdet_priv mt6377_accdet[] = {
	{
		.caps = 0,
	},
};

static const struct of_device_id mt6377_accdet_of_match[] = {
	{
...
static void cat_register(char *buf)
{
	...
	strncat(buf, accdet_log_buf, strlen(accdet_log_buf));
	st_addr = RG_AUDPREAMPLON_ADDR;
	end_addr = MT6377_AUDENC_ANA_CON32;
	for (addr = st_addr; addr <= end_addr; addr += 4) {
		idx = addr;
		ret = sprintf(accdet_log_buf,
			"(0x%x)=0x%x (0x%x)=0x%x (0x%x)=0x%x (0x%x)=0x%x\n",
...
int mt6377_accdet_read_audio_res(unsigned int res_value)
{
...
EXPORT_SYMBOL_GPL(mt6377_accdet_read_audio_res);
...
static int accdet_get_dts_data(void)
{
	...
	node = of_find_matching_node(node, mt6377_accdet_of_match);
	if (!node) {
...
void mt6377_accdet_late_init(unsigned long data)
{
...
EXPORT_SYMBOL_GPL(mt6377_accdet_late_init);

void mt6377_accdet_modify_vref_volt(void)
{
}
EXPORT_SYMBOL_GPL(mt6377_accdet_modify_vref_volt);
...
int mt6377_accdet_init(struct snd_soc_component *component,
...
EXPORT_SYMBOL_GPL(mt6377_accdet_init);

static int mt6377_accdet_probe(struct platform_device *pdev)
{
	int ret = 0;
	struct resource *res;
	const struct of_device_id *of_id =
				of_match_device(mt6377_accdet_of_match, &pdev->dev);
...
static int mt6377_accdet_remove(struct platform_device *pdev)
{
	destroy_workqueue(accdet->eint_workqueue);
	destroy_workqueue(accdet->dis_micbias_workqueue);
	destroy_workqueue(accdet->accdet_workqueue);
...
static struct platform_driver mt6377_accdet_driver = {
	.probe = mt6377_accdet_probe,
	.remove = mt6377_accdet_remove,
	.driver = {
		.name = "pmic-codec-accdet",
		.of_match_table = of_match_ptr(mt6377_accdet_of_match),
	},
};

module_platform_driver(mt6377_accdet_driver)

/* Module information */
MODULE_DESCRIPTION("MT6377 ALSA SoC accdet driver");

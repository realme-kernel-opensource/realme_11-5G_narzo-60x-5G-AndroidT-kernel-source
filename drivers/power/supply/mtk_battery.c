...
extern int oplus_set_chr_enable_otg(bool enable);
...
int check_otg_online_before_shutdown;
...
int set_shutdown_cond(struct mtk_battery *gm, int shutdown_cond)
{
	...
	case LOW_BAT_VOLT:
		if (sdc->shutdown_status.is_under_shutdown_voltage != true) {
			int i;
#ifdef OPLUS_FEATURE_CHG_BASIC
			if(check_otg_online_before_shutdown == 1 && oplus_chg_get_otg_online())
				oplus_set_chr_enable_otg(false);
#endif
			mutex_lock(&sdc->lock);
			if (now_is_kpoc != 1) {
...
static void meter_fg_30_set_battery_full(bool full)
{
	...
	if(last_full != full) {
		if (full) {
			...
			bm_err("%s [FG_INTR_CHR_FULL] send notification to fuel gauge!!!\n", __func__);
...
/*add for distinguish fuelgague and outlay-gague*/
	fg_read_dts_val(pdev->dev.of_node, "CHECK_OTG_ONLINE_BEFORE_SHUTDOWN", &(check_otg_online_before_shutdown), 1);
 	bm_err("%s, check_otg_online_before_shutdown:%d\n", __func__, check_otg_online_before_shutdown);

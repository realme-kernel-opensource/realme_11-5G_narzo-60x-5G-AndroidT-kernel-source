...
struct LCM_setting_table aod_off_10nit_cmd[] = {
	/* AOD Mode Off */
	{REGFLAG_CMD, 2, {0xFE, 0x00}},
	{REGFLAG_CMD, 2, {0x35, 0x00}},
	{REGFLAG_CMD, 1, {0x38}},
	{REGFLAG_CMD, 2, {0x51, 0x11}},
	{REGFLAG_CMD, 2, {0xFE, 0x10}},
};
...
static int lcm_setbacklight_cmdq(void *dsi, dcs_write_gce cb, void *handle, unsigned int level)
{
	...
	// }
	pr_debug("[foldable][INFO][%s:%d]will set backlight lvl:%d\n", __func__, __LINE__, level);
	...
	pr_info("[foldable][INFO][%s]backlight lvl:%d hbm_state:%d\n", __func__, level,hbm_state);
...
static int oplus_esd_backlight_recovery(void *dsi, dcs_write_gce cb, void *handle)
{
	...
	pr_debug("[foldable-LCM][INFO][%s:%d]will set backlight lvl:%d\n", __func__, __LINE__, esd_brightness);
	...
	pr_info("[foldable-LCM][INFO][%s]backlight lvl:%d\n", __func__, esd_brightness);
	return 0;
}
...
static int panel_doze_disable(struct drm_panel *panel, void *dsi, dcs_write_gce cb, void *handle)
{
	...
	if (oplus_ofp_get_fake_aod_mode()) {
		cmd = fake_aod_off_cmd;
		reg_count = sizeof(fake_aod_off_cmd) / sizeof(struct LCM_setting_table);
	} else {
		if (aod_light_mode) {
			cmd = aod_off_10nit_cmd;
			reg_count = sizeof(aod_off_10nit_cmd) / sizeof(struct LCM_setting_table);
			OFP_INFO("aod off 10nit\n");
		} else {
			cmd = aod_off_cmd;
...
static int panel_set_aod_light_mode(void *dsi, dcs_write_gce cb, void *handle, unsigned int level)
{
	...
	unsigned int reg_count = 0;
	struct LCM_setting_table *cmd = NULL;
	struct mtk_dsi *dsi_panel = (struct mtk_dsi*)dsi;
	struct lcm *ctx = panel_to_lcm(dsi_panel->panel);
	...
	if (!ctx->prepared) {
		OFP_ERR("lcm init haven't finished, can't set aod_light_mode\n");
		return 0;
	}
...
{
	char bl_tb0[] = {0xFE, 0x00};
	cb(dsi, handle, bl_tb0, ARRAY_SIZE(bl_tb0));
	pr_debug("%s esd -\n", __func__);
}
static void oplus_page_change_after_esd_read(struct drm_panel *panel, void *dsi,
                dcs_write_gce cb, void *handle)
{
	char bl_tb1[] = {0xFE, 0x10};
...
static void oplus_lcm_panel_get_data(struct lcm *ctx)
{
	...
	pr_info("0x0A return %d data(0x%08x) to dsi engine\n",ret, buffer[0] | (buffer[1] << 8));


	ret = oplus_lcm_dcs_read(ctx, 0x0E, buffer, 1);
	pr_info("return 0x0E %d data(0x%08x) to dsi engine\n",ret, buffer[0] | (buffer[1] << 8));
	ret = oplus_lcm_dcs_read(ctx, 0x0F, buffer, 1);
	pr_info("return 0x0F %d data(0x%08x) to dsi engine\n",ret, buffer[0] | (buffer[1] << 8));

	lcm_dcs_write_seq_static(ctx, 0xFE, 0x10);
}

static unsigned int lcm_get_reg_vufs18(struct drm_panel *panel)
{
	unsigned int volt = 0;
	struct lcm *ctx = panel_to_lcm(panel);

	if (regulator_is_enabled(ctx->vddio_1v8))
		/* regulator_get_voltage return volt with uV */
		volt = regulator_get_voltage(ctx->vddio_1v8);
...
void lcm_read_info(struct drm_panel *panel, int read_ic)
{
	...
	if (!IS_ERR_OR_NULL(ctx->vddio_1v8)) {
		volt = lcm_get_reg_vufs18(panel);
		pr_info("get the Regulator vufs1p8 =%d.\n",volt);
	}
	if (IS_ERR(ctx->avdden_gpio))
		DDPPR_ERR("get the enable gpio failed\n");
	else {
		ret = gpiod_get_value(ctx->avdden_gpio);
		pr_info("avdden_gpio = %d\n", ret);
	}
	if (IS_ERR(ctx->reset_gpio))
		DDPPR_ERR("get the enable gpio failed\n");
	else {
...
static struct mtk_panel_funcs ext_funcs = {
	...
	.page_change_after_esd_read = oplus_page_change_after_esd_read,
...
static int lcm_probe(struct mipi_dsi_device *dsi)
{
	...
	register_device_proc("lcd_s", "AC068_S_3_A0008", "S_3");
	strcpy(lcm_version_ext,"AC068_S_3_A0008");
	strcpy(lcm_manufacture_ext,"S_3");
#ifdef OPLUS_FEATURE_DISPLAY_ONSCREENFINGERPRINT
